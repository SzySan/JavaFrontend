...
import {SeriaAStandings} from '@/lib/SeriaA';
import {SeriaATopScorer} from '@/lib/SeriaA';
...
onMounted(() => {
  standings.value = SeriaAStandings.response[0].league.standings[0];
});
onMounted(() => {
  topScorers.value = SeriaATopScorer.response.map(player => ({
      ...
      <table class="data">
        ...
        <tbody>
          <tr v-for="team in standings" :key="team.rank">
            <td>{{ team.rank }}</td>
            <td><img :src="team.team.logo" alt="Team Logo" class="team-logo"/></td>
